...
pub fn plot(data: Vec<u32>, data2: Vec<u32>, data3: Vec<u32>) {
    let root_area = SVGBackend::new("graph.svg", (1380, 720))
    .into_drawing_area();
    ...
    for (drawing_area, idx) in drawing_areas.iter().zip(0..) {
        ...
        cc.draw_series(AreaSeries::new((0..).zip(d.iter().map(|x| *x)), 0, &col.mix(0.2)).border_style(&col)).unwrap();
    }
}
